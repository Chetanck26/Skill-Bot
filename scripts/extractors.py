...
class Extractors:
    ...
    def first_name(self, chat_text, context_data):
        context_data["context_attrs"]["first_name"] = chat_text
        return context_data

    def last_name(self, chat_text, context_data):
        context_data["context_attrs"]["last_name"] = chat_text
        return context_data

    def language_to_test(self, chat_text, context_data):
        languages = ["python"]
        for language in languages:
            search_rex = re.search(language, chat_text, flags=re.IGNORECASE)
            if search_rex:
                context_data["context_attrs"]["language_to_test"] = language.lower()
                break
        return context_data
